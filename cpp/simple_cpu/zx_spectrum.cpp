    ...
    bool execute_opcode(uint8_t opcode) {
        ...
        switch(opcode) {
            ...
            case 0x18: {// 0x18 N   (JR N        - Add signed N to PC), cycles: 12/7
                // Does not affect flags
                uint8_t pc_inc = mem_bus->read(pc+1);
                uint16_t new_pc = pc + 2;
                if ((pc_inc & 0x80) > 0) {
                    new_pc -= (pc_inc ^ 0xFF) - 1;
                } else {
                    new_pc += pc_inc;
                }
                printf("0x%04X: JR 0x%02X :: (new_pc: 0x%04X) || 0x%02X 0x%02X\n", pc, pc_inc, new_pc, opcode, flags_in);
            ...
            case 0x20: {// 0x20 N   (JR NZ, N    - If Z flag is unset, add signed N to PC), cycles: 12/7
                ...
                printf("0x%04X: JR NZ, 0x%02X :: (Z: %d, new_pc: 0x%04X) || 0x%02X 0x%02X\n", pc, pc_inc, zf, new_pc, opcode, flags_in);
                if (!zf) {
                    regs.set_reg(RegName::PC, new_pc);
                    return true;
                } else {
            ...
            case 0x23: {// 0x23     (INC HL      - Add 1 to HL), cycles: 6
                // Does not affect flags
            ...
            case 0x28: {// 0x28 N   (JR Z, N     - If Z flag is set, add signed N to PC), cycles: 12/7
                ...
                uint16_t zf = regs.get_flag(FlagName::Z);
                printf("0x%04X: JR Z, 0x%02X :: (Z: %d, new_pc: 0x%04X) || 0x%02X 0x%02X\n", pc, pc_inc, zf, new_pc, opcode, flags_in);
                if (zf) {
                    regs.set_reg(RegName::PC, new_pc);
                    return true;
                } else {
            ...
            case 0x2B: {// 0x2B     (DEC HL      - Subtracts one from HL), cycles: 6
                // Does not affect flags
            ...
            case 0x30: {// 0x30     (JR NC, N    - If carry flag is unset, add signed N to PC), cycles: 12/7
                ...
                printf("0x%04X: JR NC, 0x%02X :: (C: %d, new_pc: 0x%04X) || 0x%02X 0x%02X\n", pc, pc_inc, cf, new_pc, opcode, flags_in);
                if(!cf) {
                    regs.set_reg(RegName::PC, new_pc);
                    return true;
                }
            ...
            case 0x36: {// 0x36     (LD (HL), N  - Load N into (HL)), cycles: 10
                ...
                uint16_t val = mem_bus->read(pc+1);
                uint16_t addr = regs.get_reg(RegName::HL);
                printf("0x%04X: LD (HL), 0x%02X :: (addr = 0x%04X) || 0x%02X\n", pc, val, addr, opcode);
            ...
            case 0x38: {// 0x38     (JR C, N     - If carry flag is set, add signed N to PC), cycles: 12/7
                ...
                uint16_t cf = regs.get_flag(FlagName::C);
                printf("0x%04X: JR NC, 0x%02X :: (C: %d, new_pc: 0x%04X) || 0x%02X\n", pc, pc_inc, cf, new_pc, opcode);
                if(cf) {
                    regs.set_reg(RegName::PC, new_pc);
                    return true;
                }
            ...
            case 0x3B: {// 0x3B     (DEC SP      - Subtracts one from SP), cycles: 6
                // Does not affect flags
            ...
            case 0x6F: {// 0x6F     (LD L, A     - Load A into L), cycles: 4
                ...
                regs.set_reg(RegName::L, regs.get_reg(RegName::A));
                break;
            }
            ...
            case 0x80: {// 0x80     (ADD A, B    - Adds B to A), cycles: 4
                // C as defined
                // N reset
                // P/V detects overflow
                // H as defined
            ...
            case 0x81: {// 0x81     (ADD A, C    - Adds C to A), cycles: 4
                // C as defined
                // N reset
                // P/V detects overflow
                // H as defined
            ...
            case 0x83: {// 0x83     (ADD A, E    - Adds E to A), cycles: 4
                // C as defined
                // N reset
                // P/V detects overflow
                // H as defined
            ...
            case 0x84: {// 0x84     (ADD A, H    - Adds H to A), cycles: 4
                // C as defined
                // N reset
                // P/V detects overflow
                // H as defined
            ...
            case 0x85: {// 0x85     (ADD A, L    - Adds L to A), cycles: 4
                // C as defined
                // N reset
                // P/V detects overflow
                // H as defined
            ...
            case 0x86: {// 0x86     (ADD A, (HL) - Adds (HL) to A), cycles: 7
                // C as defined
                // N reset
                // P/V detects overflow
                // H as defined
            ...
            case 0x87: {// 0x87     (ADD A, A    - Adds A to A), cycles: 4
                // C as defined
                // N reset
                // P/V detects overflow
                // H as defined
            ...
            case 0x90: {// 0x90     (SUB A, B    - Subtracts B from A), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: SUB A, B :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0x91: {// 0x91     (SUB A, C    - Subtracts C from A), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: SUB A, C :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0x92: {// 0x92     (SUB A, D    - Subtracts D from A), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: SUB A, D :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0x93: {// 0x93     (SUB A, E    - Subtracts E from A), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: SUB A, E :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0x94: {// 0x94     (SUB A, H    - Subtracts H from A), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: SUB A, H :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0x95: {// 0x95     (SUB A, L    - Subtracts L from A), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: SUB A, L :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0x96: {// 0x96     (SUB A, (HL) - Subtracts (HL) from A), cycles: 7
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
            ...
            case 0x97: {// 0x97     (SUB A, A    - Subtracts A from A), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
            ...
            case 0x98: {// 0x98     (SBC A, B    - Subtracts B and the carry flag from A), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: SBC A, B :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0x99: {// 0x99     (SBC A, C    - Subtracts C and the carry flag from A), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: SBC A, C :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0x9A: {// 0x9A     (SBC A, D    - Subtracts D and the carry flag from A), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: SBC A, D :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0x9B: {// 0x9B     (SBC A, E    - Subtracts E and the carry flag from A), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: SBC A, E :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0x9C: {// 0x9C     (SBC A, H    - Subtracts H and the carry flag from A), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: SBC A, H :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0x9D: {// 0x9D     (SBC A, L    - Subtracts L and the carry flag from A), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: SBC A, L :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0x9E: {// 0x9E     (SBC A, (HL) - Subtracts (HL) and the carry flag from A), cycles: 7
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: SBC A, (HL) :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0x9F: {// 0x9F     (SBC A, A    - Subtracts A and the carry flag from A), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: SBC A, A :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            ...
            case 0xA1: {// 0xA1     (AND A, C    - Bitwise AND on A with C), cycles: 4
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: AND A, C :: (0x%02X & 0x%02X = 0x%02X) || 0x%02X 0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0xA2: {// 0xA2     (AND A, D    - Bitwise AND on A with D), cycles: 4
                // C reset
                // N reset
                // P/V detects parity
                // H set
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: AND A, D :: (0x%02X & 0x%02X = 0x%02X) || 0x%02X 0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0xA3: {// 0xA3     (AND A, E    - Bitwise AND on A with E), cycles: 4
                // C reset
                // N reset
                // P/V detects parity
                // H set
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: AND A, E :: (0x%02X & 0x%02X = 0x%02X) || 0x%02X 0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0xA4: {// 0xA4     (AND A, H    - Bitwise AND on A with H), cycles: 4
                // C reset
                // N reset
                // P/V detects parity
                // H set
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: AND A, H :: (0x%02X & 0x%02X = 0x%02X) || 0x%02X 0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0xA5: {// 0xA5     (AND A, L    - Bitwise AND on A with L), cycles: 4
                // C reset
                // N reset
                // P/V detects parity
                // H set
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: AND A, L :: (0x%02X & 0x%02X = 0x%02X) || 0x%02X 0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0xA6: {// 0xA6     (AND A, (HL) - Bitwise AND on A with (HL)), cycles: 7
                // C reset
                // N reset
                // P/V detects parity
                // H set
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: AND A, (HL) :: (0x%02X & 0x%02X = 0x%02X) || 0x%02X 0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            case 0xA7: {// 0xA7     (AND A, A    - Bitwise AND on A with A), cycles: 4
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: AND A, A :: (0x%02X & 0x%02X = 0x%02X) || 0x%02X 0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                break;
            }
            ...
            case 0xB8: {// 0xB8     (CP A, B     - Subtracts B from A and affects flags according to the result. A is not modified.), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: CP A, B :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                break;
            }
            case 0xB9: {// 0xB9     (CP A, C     - Subtracts C from A and affects flags according to the result. A is not modified.), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: CP A, C :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                break;
            }
            case 0xBA: {// 0xBA     (CP A, D     - Subtracts D from A and affects flags according to the result. A is not modified.), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: CP A, D :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                break;
            }
            case 0xBB: {// 0xBB     (CP A, E     - Subtracts E from A and affects flags according to the result. A is not modified.), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: CP A, E :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                break;
            }
            case 0xBC: {// 0xBC     (CP A, H     - Substract H from A and update flags. A stays unchanged), cycles: 4
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: CP A, H :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                break;
            }
            case 0xBD: {// 0xBD     (CP A, L     - Substract L from A and update flags. A stays unchanged), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: CP A, L :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                break;
            }
            case 0xBE: {// 0xBE     (CP A, (HL)  - Substract (HL) from A and update flags. A stays unchanged), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: CP A, (HL) :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                break;
            }
            case 0xBF: {// 0xBF     (CP A, A     - Substract A from A and update flags. A stays unchanged), cycles: 4
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: CP A, A :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, a, val, res, opcode, regs.get_reg(RegName::F));
                break;
            }
            ...
            case 0xC1: {// 0xC1     (POP BC      - Pop value from stack and put it into BC), cycles: 10
                // Does not affect flags
            ...
            case 0xC5: {// 0xC5     (PUSH BC     - Push BC onto stack), cycles: 11
                // Does not affect flags
            ...
            case 0xC9: {// 0xC9     (RET         - The top stack entry is popped into PC), cycles: 10
                // Does not affect flags
            ...
            case 0xD1: {// 0xD1     (POP DE      - Pop value from stack and put it into DE), cycles: 10
                // Does not affect flags
            ...
            case 0xD3: {// 0xD3 N   (OUT A, N    - Write the value of A into port N), cycles: 11
                // Does not affect flags
                uint16_t a = regs.get_reg(RegName::A);
            ...
            case 0xD5: {// 0xD5     (PUSH DE     - Push DE onto stack), cycles: 11
                // Does not affect flags
            ...
            case 0xDD: {// 0xDD     (IX Instructions, read one more byte to get the actual opcode)
                ...
                if (new_opcode == 0x09) {          // 0xDD 0x19     (ADD IX, BC  - Add BC to IX), cycles: 15
                    ...
                    num_bytes_read += 1;
                } else if (new_opcode == 0x19) {   // 0xDD 0x19     (ADD IX, DE  - Add DE to IX), cycles: 15
                    // C as defined
                    // N reset
                    // P/V unaffected
                    // H as defined
                    ...
                    regs.set_reg(RegName::IX, res);
                    num_bytes_read += 1;
                } else if (new_opcode == 0x21) {   // 0xDD 0x21 N N (LD IX, NN - Loads nn into register IX), cycles: 14
                    ...
                    num_bytes_read += 3;
                } else if (new_opcode == 0x22) {   // 0xDD 0x22 N N (LD (NN), IX - Stores IX into the memory location pointed to by nn), cycles: 20
                    ...
                    num_bytes_read += 3;
                } else if (new_opcode == 0x39) {   // 0xDD 0x39     (ADD IX, SP  - Add SP to IX), cycles: 15
                    // C as defined
                    // N reset
                    // P/V unaffected
                    // H as defined
                    ...
                    regs.set_reg(RegName::A, res);
                    num_bytes_read += 2;
                } else if (new_opcode == 0x96) {   // 0xDD 0x96 D   (SUB A, (IX+D) - Subtracts the value pointed to by IX plus d from A), cycles: 19
                    // C as defined
                    // N set
                    // P/V detects overflow
                    // H as defined
                    ...
                    regs.set_reg(RegName::A, res);
                    num_bytes_read += 2;
                } else if (new_opcode == 0x9E) {   // 0xDD 0x9E D   (SBC A, (IX+D) - Subtracts the value pointed to by IX plus d and the carry flag from A), cycles: 19
                    // C as defined
                    // N set
                    // P/V detects overflow
                    // H as defined
                    ...
                    regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                    regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                    printf("0x%04X: SBC A, (IX + 0x%02X) :: (0x%02X - 0x%02X -> 0x%02X, addr: 0x%04X) || 0x%02X 0x%02X, F=0x%02X\n", pc, offset, a, val, res, res_addr, opcode, new_opcode, regs.get_reg(RegName::F));
                    regs.set_reg(RegName::A, res);
                    num_bytes_read += 2;
                } else if (new_opcode == 0xA6) {   // 0xDD 0xA6 D   (AND A, (IX+D) - Bitwise AND on A with the value pointed to by IX plus d), cycles: 19
                    // C as defined
                    // N set
                    // P/V detects overflow
                    // H as defined
                    ...
                    regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                    regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                    printf("0x%04X: AND A, (IX + 0x%02X) :: (0x%02X & 0x%02X -> 0x%02X, addr: 0x%04X) || 0x%02X 0x%02X, F=0x%02X\n", pc, offset, a, val, res, res_addr, opcode, new_opcode, regs.get_reg(RegName::F));
                    regs.set_reg(RegName::A, res);
                    num_bytes_read += 2;
                } else if (new_opcode == 0xAE) {   // 0xDD 0xAE D   (XOR A, (IX+D) - Bitwise XOR on A with the value pointed to by IX plus d), cycles: 19
                    // C as defined
                    // N set
                    // P/V detects overflow
                    // H as defined
            ...
            case 0xDE: {// 0xDE N   (SBC A, N - Subtracts N and the carry flag from A), cycles: 7
                // C as defined
                // N set
                // P/V detects overflow
                // H as defined
                ...
                regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                printf("0x%04X: SBC A, 0x%02X :: (0x%02X-0x%02X=0x%02X) || 0x%02X F=0x%02X\n", pc, val, a, val, res, opcode, regs.get_reg(RegName::F));
                regs.set_reg(RegName::A, res);
                num_bytes_read += 1;
                break;
            }
            case 0xDF: {// 0xDF     (RST 0x18   - The current PC value plus one is pushed onto the stack, then is loaded with 24.), cycles: 11
                // Does not affect flags
                regs.set_reg(RegName::PC, pc+3);    // Increment PC to point to the next instruction
                printf("0x%04X: RST 0x18 || 0x%02X\n", pc, opcode);
                push_reg(RegName::PC);
                regs.set_reg(RegName::PC, 0x18);
                return true;
            }
            ...
            case 0xE1: {// 0xE1     (POP HL      - Pop value from stack and put it into HL), cycles: 10
                // Does not affect flags
            ...
            case 0xED: {// 0xED     (MISC intruction, read one more byte to get the actual opcode)
                ...
                if (new_opcode == 0x42) {          // 0xED 0x42     (SBC HL, BC  - Subtracts BC and the carry flag from HL), cycles: 15
                    ...
                    alu.perform_operation(ALU_Op:: SUB, 16);
                    uint16_t res = alu.get_res();
                    regs.update_flag(FlagName::C, alu.get_flag(ALU_Flags::C));
                    regs.set_flag(FlagName::N);
                    regs.update_flag(FlagName::P_V, alu.get_flag(ALU_Flags::O));
                    ...
                    regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                    regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                    printf("0x%04X: SBC HL, BC :: (0x%04X - 0x%04X = 0x%04X) || 0x%02X 0x%02X\n", pc, hl, val, res, opcode, regs.get_reg(RegName::F));
                    regs.set_reg(RegName::HL, res);
                    num_bytes_read += 1;
                } else if (new_opcode == 0x43) {   // 0xED 0x43 N N (LD (NN), BC - Load BC into memory pointed by NN), cycles: 20
                    // Does not affect flags
                    uint16_t addr = mem_read_16b(pc+2);
                    ...
                    regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                    regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                    printf("0x%04X: SBC HL, DE :: (0x%04X - 0x%04X = 0x%04X) || 0x%02X 0x%02X\n", pc, hl, val, res, opcode, regs.get_reg(RegName::F));
                    regs.set_reg(RegName::HL, res);
                    num_bytes_read ++;
                } else if (new_opcode == 0x57) {   // 0xED 0x57     (LD A, I     - Stores the value of register I into A.), cycles: 9
                    // C unaffected
                    // N reset
                    // P/V exceptional (Set (1) if interrupts are currently enabled; Reset (0) if interrupts are disabled.)
                    // H reset
                    // Z as defined
                    // S as defined
                    uint16_t val = regs.get_reg(RegName::I);
                    regs.clear_flag(FlagName::N);
                    regs.clear_flag(FlagName::H);
                    regs.update_flag(FlagName::P_V, interrupts_enabled);
                    regs.update_flag(FlagName::Z, val == 0);
                    regs.update_flag(FlagName::S, val & 0x80 != 0);
                    printf("0x%04X: LD A, I || 0x%02X 0x%02X\n", pc, opcode, regs.get_reg(RegName::F));
                    regs.set_reg(RegName::A, val);
                    ...
                    regs.set_reg(RegName::HL, res);
                    num_bytes_read += 1;
                } else if (new_opcode == 0x62) {   // 0xED 0x62     (SBC HL, HL  - Subtract HL and Carry from HL), cycles 15
                    // C as defined
                    // N reset
                    // P/V detects overflow
                    // H as defined
                    ...
                    regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                    regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                    printf("0x%04X: SBC HL, HL :: (0x%04X - 0x%04X = 0x%04X) || 0x%02X 0x%02X\n", pc, hl, val, res, opcode, regs.get_reg(RegName::F));
                    regs.set_reg(RegName::HL, res);
                    num_bytes_read ++;
                    ...
                    regs.set_reg(RegName::HL, res);
                    num_bytes_read += 1;
                } else if (new_opcode == 0x72) {   // 0xED 0x72     (SBC HL, SP  - Subtract SP and Carry from HL), cycles 15
                    // C as defined
                    // N reset
                    // P/V detects overflow
                    // H as defined
                    ...
                    regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                    regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                    printf("0x%04X: SBC HL, SP :: (0x%04X - 0x%04X = 0x%04X) || 0x%02X 0x%02X\n", pc, hl, val, res, opcode, regs.get_reg(RegName::F));
                    regs.set_reg(RegName::HL, res);
                    num_bytes_read ++;
            ...
            case 0xFD: {// 0xFD     (IY Instructions, read one more byte to get the actual opcode)
                ...
                if (new_opcode == 0x09) {          // 0xFD 0x19     (ADD IY, BC    - Add BC to IY), cycles: 15
                    ...
                    num_bytes_read += 1;
                } else if (new_opcode == 0x19) {   // 0xFD 0x19     (ADD IY, DE    - Add DE to IY), cycles: 15
                    // C as defined
                    // N reset
                    // P/V unaffected
                    // H as defined
                    ...
                    regs.set_reg(RegName::IY, res);
                    num_bytes_read += 1;
                } else if (new_opcode == 0x21) {   // 0xFD 0x21 N N (LD IY, NN     - Loads nn into register IY), cycles: 14
                    uint16_t value = mem_read_16b(pc+2);
                    printf("0x%04X: LD IY, 0x%04X || 0x%02X 0x%02X\n", pc, value, opcode, new_opcode);
                    regs.set_reg(RegName::IY, value);
                    num_bytes_read += 3;
                } else if (new_opcode == 0x22) {   // 0xFD 0x22 N N (LD (NN), IY   - Stores IY into the memory location pointed to by nn), cycles: 20
                    uint16_t addr = mem_read_16b(pc+2);
                    ...
                    mem_bus->write(res_addr, val);
                    num_bytes_read += 3;
                } else if (new_opcode == 0x39) {   // 0xFD 0x39     (ADD IY, SP    - Add SP to IY), cycles: 15
                    // C as defined
                    // N reset
                    // P/V unaffected
                    // H as defined
                    ...
                    regs.set_reg(RegName::A, res);
                    num_bytes_read += 2;
                } else if (new_opcode == 0x96) {   // 0xFD 0x96 D   (SUB A, (IY+D) - Subtracts the value pointed to by IY plus d from A), cycles: 19
                    // C as defined
                    // N set
                    // P/V detects overflow
                    // H as defined
                    ...
                    regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                    regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                    printf("0x%04X: SUB A, (IY + 0x%02X) :: (0x%02X + 0x%02X -> 0x%02X, addr: 0x%04X) || 0x%02X 0x%02X, F=0x%02X\n", pc, offset, a, val, res, res_addr, opcode, new_opcode, regs.get_reg(RegName::F));
                    regs.set_reg(RegName::A, res);
                    num_bytes_read += 2;
                } else if (new_opcode == 0x9E) {   // 0xFD 0x9E D   (SBC A, (IY+D) - Subtracts the value pointed to by IY plus d and the carry flag from A), cycles: 19
                    // C as defined
                    // N set
                    // P/V detects overflow
                    // H as defined
                    ...
                    regs.update_flag(FlagName::Z, alu.get_flag(ALU_Flags::Z));
                    regs.update_flag(FlagName::S, alu.get_flag(ALU_Flags::S));
                    printf("0x%04X: SBC A, (IY + 0x%02X) :: (0x%02X + 0x%02X -> 0x%02X, addr: 0x%04X) || 0x%02X 0x%02X, F=0x%02X\n", pc, offset, a, val, res, res_addr, opcode, new_opcode, regs.get_reg(RegName::F));
                    regs.set_reg(RegName::A, res);
                    num_bytes_read += 2;
                } else if (new_opcode == 0xA6) {   // 0xFD 0xA6 D   (AND A, (IY+D) - Bitwise AND on A with the value pointed to by IY plus d), cycles: 19
                    // C as defined
                    // N set
                    // P/V detects overflow
                    // H as defined
                    ...
                    num_bytes_read += 2;
                } else if (new_opcode == 0xCB) {   // // 0xFD       (IY CB Instructions, read one more byte to get the actual opcode)
                    uint16_t new_fdcb_opcode = mem_bus->read(pc+2);
                    FD_CB_opcode_counter[new_fdcb_opcode] ++;
                    if (new_opcode == 0xFF) {
                        ;
                    } else {
                    printf("Unknown IY opcode: 0xFD CB 0x%X\n", new_fdcb_opcode);
                    return false;
                }
...
int main() {
    ...
    if(rom.map_image("./zx_spectrum/spec48.rom", 0)) {
        std::cout << "spec48 successfully mapped\n";
    } else {
